    ...
    def nbcf(self):
        ...
        for movie in range(self.movies):
            ...
            for qualified in self.qualified_array:
                ...
                self.pi[movie, qualified] = np.log(
                    (
                        len(self.m_collaborative_filtering[(movie, qualified)])
                        + self.alpha
                    )
                    / d
                )
        ...
        for user in range(self.users):
            ...
            for qualified in self.qualified_array:
                ...
                self.pu[user, qualified] = np.log(
                    (
                        len(self.u_collaborative_filtering[(user, qualified)])
                        + self.alpha
                    )
                    / d
                )
        ...
        for movie in range(self.movies):
            ...
            for qualified in self.qualified_array:
                ...
                for j_movie in self.user_map[user]:
                    ...
                    tmp += np.log(numerator + self.alpha) - np.log(
                        denominator + self.r_alpha
                    )
        ...
        for user in range(self.users):
            for movie in range(self.movies):
                ...
                for qualified in self.qualified_array:
                    ...
                    left = (
                        self.user_prediction[user, movie, qualified]
                        * (1 / (1 + ui))
                        # + self.EPSILON
                    )
                    right = (
                        self.item_prediction[user, movie, qualified]
                        * (1 / (1 + iu))
                        # + self.EPSILON
                    )
